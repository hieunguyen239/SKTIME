    ...
    public class OutcomeController : BaseController
    {
        ...
        public ActionResult Create()
        {
            if (Request.IsAjaxRequest())
                return Json(new
                {
                    html = RenderPartialViewToString(Views.SavePartial, new OutcomeInfo(Employee.BussinessID, Employee.ID))
                }, JsonRequestBehavior.AllowGet);
            return View(Views.Save, new OutcomeInfo(Employee.BussinessID, Employee.ID));
        }
        [LoginFilter]
        [HttpGet]
        ...
        private void SaveDownload(string fileName, List<OutcomeInfo> products)
        {
